...
** TODO [#C] Getting OSX attributes on a file sometimes fails - maybe fixed.

* Refactor
** Split Book and file code
** Make md5 file names, dirs, defined in md5 module
** Move md5 functions into shared module. Move all MD5 code into a subdir including tests
** Cleanup files - delete old test files, organize, make names consistent.
** Release process for apps & lib into bin & released Lib
** Better - smarter - save dupes, load - not repeat counts
** Rework code in MooNode for is readable, other values from stats - parse stats directly
** Enable NodeTree insert to insert a whole tree for a Tree merge

* ToDo

** Give collection a attached dir on new
** Implement save to packed file
** Fix / Test Collect Tree
   + Scan tree
   + Warn if Dir out of date, don't fix
   + Save file at root of tree
   + Save copy of tree on data dir
     * Naming method for a tree in dir?
   + Rotate old copy of tree
   + Save a list of all trees scanned in data dir? Build from datafiles in dir?
   + Delete any old subtree files found? In Tree or data dir?
   + Make sense to have a one dir version?
   + Create 
** Finish MooFile
   + Tests
