...
my $dir = "/Users/tshott/Downloads/_ebook";

$dir = shift(@ARGV);

# my $rule = File::Find::Rule->directory->start($dir);
# while ( defined ( my $node = $rule->match ) ) {
#     next if ($node =~ m!/\.!);

#     say "$node";
# }

my $rule = File::Find::Rule->directory->name('.unwanted')->start($dir);
while ( defined ( my $node = $rule->match ) ) {
    say "$node";
#    unlink($node);
#    rmtree $node;
